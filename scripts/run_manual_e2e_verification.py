...
def run_verification():
    ...
    try:
        ...
        subprocess.run(
            ["kubectl", "exec", "statefulset/my-release-localmind-postgres", "--", "psql", "-U", "user", "-d", "localmind", "-c", cleanup_sql],
            check=True,
        )
        subprocess.run(["kubectl", "rollout", "restart", "deployment/my-release-localmind-api"], check=True)
        subprocess.run(["kubectl", "rollout", "status", "deployment/my-release-localmind-api"], check=True)
        print("Database seeded with fresh default credentials.")

        print("Starting port forwards...")
        pf_web = subprocess.Popen(["kubectl", "port-forward", "--address", "127.0.0.1,::1", "deployment/my-release-localmind-web", "8080:3000"])
        pf_api = subprocess.Popen(["kubectl", "port-forward", "--address", "127.0.0.1,::1", "deployment/my-release-localmind-api", "8000:8000"])
        
        # Give port forwards a second to bind
        time.sleep(2)
        ...
        with sync_playwright() as p:
            ...
            page.wait_for_selector("#nav-upload-btn", timeout=15000)
            ...
            for q_label, q_text, screenshot_name in questions:
                ...
                messages = page.query_selector_all(".prose")
        ...
        if pf_web: pf_web.terminate()
        if pf_api: pf_api.terminate()
